...
const MATCH_SCENE: PackedScene = preload("res://gameplay/match/match.tscn")
...
func show_main_menu() -> void:
	...
	menu.solo_requested.connect(start_solo_match)
	menu.quit_requested.connect(_quit_game)


func start_solo_match() -> void:
	var match_scene := MATCH_SCENE.instantiate() as RefereeMatch
